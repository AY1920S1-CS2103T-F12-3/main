...
When the user enters an incomplete command, most of the parameters could have been already provided. Instead of forcing the user to edit the command entirely, TagLine will prompt the user for further details instead.
...
The prompting mechanism uses `Prompt` objects to represent individual queries for additional information. A list of `Prompt` objects is used to pass information between the `Parser` and `MainWindow`. `Prompt` implements the following operations:
...
Step 1: The `Ui` passes the user's command to `Logic`, which finds one or more missing compulsory fields. For each missing field, it creates a new `Prompt` object with a qestion. Then it throws a `ParseException` containing the list of `Prompt` objects.
...
The user can also abort the command by entering a special string. Then the `Ui` will discard the original command and continue to receive further user commands.
