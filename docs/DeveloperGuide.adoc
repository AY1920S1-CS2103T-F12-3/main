...
. The user is currently being prompted, and enters some input to answer a prompt. There are no more prompts remaining.
    ...
    .. `MainWindow` calls `getPendingCommand` and `getNextPrompt` of `promptHandler`.
...
* **Alternative 3**: `TaglineParser` and the individual parser classes handles the `Prompt` objects when parsing the command
